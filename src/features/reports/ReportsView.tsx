...
export function ReportDocument({ projection, demoMode, BrandComponent, StarsComponent }: ReportDocumentProps) {
  const { locationReports, isCombinedReport, reportBusiness, metrics } = projection;
  ...
  return (
    ...
    <article className="report-paper">
      ...
      <section className="report-unavailable-outcome" aria-labelledby="mobilewan-usage-title">
        ...
        <p>Usage requires a managed GPU benchmark, legal and moderation clearance, private Storage, a commercial-account projection and append-only credit ledger, and approved pricing.</p>
      </section>
...
export function ReportsView({
  ...
  const projection = buildReportProjection(business, selectedBusiness, combined);
  ...
  return (
    <div className="view-stack">
      ...
      <section className="report-shell">
        <header className="report-toolbar"><div><span>{demoMode ? "Monthly report" : "Operational snapshot"}</span><strong>{demoMode ? "July 2026" : new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "long", year: "numeric" }).format(new Date())}</strong></div><div className="report-toolbar__actions">{canCombineReportLocations(business) && <div className="report-scope-toggle" aria-label="Report scope"><button type="button" className={!combined ? "is-active" : undefined} onClick={() => setCombined(false)}>Selected location</button><button type="button" className={combined ? "is-active" : undefined} onClick={() => setCombined(true)}>All locations</button></div>}<ButtonComponent variant="secondary" onClick={printReport}><Printer size={16} /> Print report</ButtonComponent></div></header>
        <ReportDocument projection={projection} demoMode={demoMode} BrandComponent={BrandComponent} StarsComponent={StarsComponent} />
      </section>
    </div>
  );
}
