...
test("support-role operators may open only view support sessions and share the database reason boundary", async () => {
  ...
  assert.match(source, /\["agency_admin",\s*"agency_user"\]\.includes\(actor\.role\)/);
...
test("Google mutations require business management while selection reads remain readable", async () => {
  ...
  assert.match(google, /function requireDirectBusinessGoogleActor[\s\S]{0,260}GOOGLE_DIRECT_BUSINESS_REQUIRED/);
  assert.match(google, /oauth\/start"[\s\S]{0,400}const actor = requireActor\(request\);\s*requireDirectBusinessGoogleActor\(actor\);/);
  assert.match(google, /app\.post\("\/api\/v1\/integrations\/google\/oauth\/selections\/:selectionToken"[\s\S]{0,400}const actor = requireActor\(request\);\s*requireDirectBusinessGoogleActor\(actor\);/);
