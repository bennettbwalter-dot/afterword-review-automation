...
create or replace function app_private.list_active_agency_client_grants(p_agency_id uuid) returns table (id uuid,business_id uuid,location_id uuid,permissions text[],expires_at timestamptz) language sql stable security definer set search_path=pg_catalog as $$ select grant.id,grant.business_id,grant.location_id,grant.permissions,grant.expires_at from public.agency_client_grants grant where grant.agency_id=p_agency_id and grant.status='active' and (grant.expires_at is null or grant.expires_at>statement_timestamp()) and app_private.current_support_session_id() is null and app_private.current_agency_role(p_agency_id) is not null $$;
