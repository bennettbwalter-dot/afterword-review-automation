...
export class PostgresRepository implements PlatformRepository {
  ...
  async revokeCurrentClientAgencyGrant(actor: ActorContext, grantId: string, businessId: string, correlationId: string) {
    return new AgencyGrantPostgres(this.runtime()).revokeAsClient(actor, grantId, businessId, correlationId);
  }
  ...
  async issueAgencyClientAccessClaim(actor: ActorContext, agencyId: string, email: string, permissions: import("../agency/types.js").AgencyGrantPermission[], selfApproverUserId: string | undefined, tokenHash: Buffer, expiresAt: Date, correlationId: string) { await new AgencyGrantPostgres(this.runtime()).issueAccessClaim(actor, agencyId, email, permissions, selfApproverUserId, tokenHash, expiresAt, correlationId); }
