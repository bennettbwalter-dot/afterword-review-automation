...
export class AgencyGrantPostgres {
  ...
  async issueAccessClaim(actor: ActorContext, email: string, permissions: AgencyGrantPermission[], tokenHash: Buffer, expiresAt: Date, correlationId: string) { await withActorTransaction(this.runtimePool, actor, (client) => client.query("select app_private.issue_agency_client_access_claim($1,$2,$3,$4,$5,$6,$7,$8,$9)", [email, permissions, null, null, null, null, tokenHash, expiresAt, correlationId]).then(() => undefined)); }
