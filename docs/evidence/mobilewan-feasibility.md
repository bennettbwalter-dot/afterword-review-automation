...
Run `scripts/mobilewan/benchmark.ps1` only on an owner-supplied managed host with exactly one visible NVIDIA A100 80 GB GPU against the exact local snapshots. Before any sample command, it requires an owner-supplied offline provenance JSON file with the two exact revisions and one SHA-256 entry for every snapshot file; directory names and marker files alone are not proof. The run must use at least ten representative promotional prompts with fixed integer seeds and record cold/warm latency, peak VRAM, technically-valid-output rate, output checksums, retries/concurrency, and cost per technically valid clip. The wrapper does not download weights or select a provider. A mixed-GPU, lower-VRAM, or non-A100 host cannot emit a qualifying artifact.
